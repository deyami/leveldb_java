...
public class crc32java {
    ...
    public int Extend(int init_crc, byte[] data, int n) {
        crc32val = init_crc;
        update(data, 0, n);
        return crc32val;
    }
    ...
    public int Value(byte[] data, int length) {
        reset();
        update(data, 0, length);
        return crc32val;
    }

    // Return the crc32c of data[ofs, ofs + n- 1]
    public int Value(byte[] data, int offset, int length) {
        reset();
        update(data, offset, length);
        return crc32val;
    }

    static int kMaskDelta = 0xa282ead8;
    static int low15 = 0x7fff;
    static int low17 = 0x1ffff;
...
//		System.out.println(r);
        ...
        int msk = Mask(init);
        int umsk = Unmask(msk);
        System.out.println(init + ", " + msk + ", " + umsk);
    }
}
